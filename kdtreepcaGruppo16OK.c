...
void assegnaRegioni(KDTREE albero, int k){
    if(albero->left ==NULL && albero->right==NULL){
        exit(-1);
    }
    if(cont==0){
        albero->H=build_region_radice(albero, k);
        if(albero->left!=NULL){
            albero->left->H=build_region_figlio(albero->left, albero->point, albero->cut_dim, albero->H, k);
        }
        if(albero->right!=NULL){
            albero->right->H=build_region_figlio(albero->right, albero->point, albero->cut_dim, albero->H, k);
        }
        cont++;
